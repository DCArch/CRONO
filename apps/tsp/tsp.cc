...
int _WEIGHTS[5][5] = {{INF, 2,   3, 1, INF},
                      {INF, INF, 5, 7,   4},
                      {3,   1, INF, 6,   1},
                      {8,   2,   4, INF, 3},
                      {7,   INF, 6, 1, INF}};
...
class PartialTour
{
    public:
        ...
        const vector<int>& getUnvisited() const
        {
            return _unvisited;
        }
...
PartialTour::PartialTour(int NUM_CITIES, int start_city) : _cost(0), _length(1)
{
...
PartialTour::PartialTour(const PartialTour& tour) :
        _path(tour.getPath()), _unvisited(tour.getUnvisited()), _cost(tour.getCost()), _length(tour.getLength())
{
}
...
void PartialTour::addCity(int idx)
{
    assert(_length >= 1);
...
ThreadData::ThreadData(int tid, WorkQueue& work_queue, int NUM_CITIES) :
        _tid(tid), _work_queue(work_queue), _NUM_CITIES(NUM_CITIES)
{
...
void doWork(int tid, WorkQueue& work_queue, PartialTour* tour, PartialTour* best_tour, int NUM_CITIES)
{
...
int getNumLevels(int NUM_THREADS, int NUM_CITIES)
{
...
void generatePartialTours(PartialTour* tour, int num_levels, vector<PartialTour*>& partial_tour_vec)
{
    int length = tour->getLength();
...
void dividePartialTours(const vector<PartialTour*>& partial_tour_vec, vector <WorkQueue>& work_queue_vec)
{
    int queue_id = 0;
